...
void  IRsend::sendMyRCProtocol (unsigned long data,  int nbits)
{
...
bool  IRrecv::decodeMyRCProtocol (decode_results *results)
{
...
//	for (int i = 0;  i < SHUZU_BITS;  i++) {
	...
	results->bits        = BITS;
	results->value       = data;
	results->decode_type = MYRCPROTOCOL;
	return true;
}
#endif
...
bool  IRrecv::decodeMyRCProtocol (decode_results *results)
{
	...
	if ( (irparams.rawlen == 4)
	    && MATCH_SPACE(results->rawbuf[offset  ], RPT_SPACE)
	    && MATCH_MARK (results->rawbuf[offset+1], BIT_MARK )
	   ) {
		results->bits        = 0;
	...
	if (irparams.rawlen < (2 * BITS) + 4)  return false ;
